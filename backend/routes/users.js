...
router.post("/users/signup", async (req, res) => {
  ...
  if (!passwordPattern.test(password)) {
    ...
    return sendErr(res, '密碼不符合規則，需要包含英文數字大小寫，最短8個字，最長16個字', 400)
  }
...
router.put("/users/profile", verifyToken, async (req, res, next) => {
  ...
  try {
    ...
    const result = await pool.query(`
      UPDATE users SET name = $1 WHERE id = $2 RETURNING name, email`, [nameWoSpace, userId])
...
router.put("/users/password", verifyToken, async (req, res, next) => {
  ...
  if (newPwd != confirmPwd) return sendErr(res, '新密碼與驗證新密碼不一致', 400)
  ...
  try {
    ...
    await pool.query(`
      UPDATE users SET password = $1 WHERE id = $2`, [hashedPwd, userId])
